...
export class HederaSdk {
      ...
      for (let i = 0; i < nbOfChunk; i++) {
        ...
        for (let i = 0; i < limit_chunk; i++) {
          mintTransaction.addMetadata(
            Buffer.from(`https://cloudflare-ipfs.com/ipfs/${cid}`)
          );
        }
  ...
  async checkBalance() {
    ...
    if (balance < hederaFees.hbar) {
      const err =
        "You don't have enough money available in your account :: Remaining :: " +
        balance +
        " :: Price :: " +
        hederaFees;
      Logger.error(err);
      ...
      return operation.then(resolve).catch((reason) => {
        if (retries > 0) {
          return (
            this.wait(delay)
              .then(
                this.retryOperation.bind(null, operation, delay, retries - 1)
              )
              // @ts-ignore
              .then(resolve)
              .catch(reject)
